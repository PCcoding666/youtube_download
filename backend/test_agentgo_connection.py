...
async def test_agentgo_connection():
    ...
    # 1. 检查配置
    print("1. 配置检查:")
    print(f"   API Key: {settings.agentgo_api_key[:20]}..." if settings.agentgo_api_key else "   API Key: 未设置")
    print(f"   API URL: {settings.agentgo_api_url}")
    print(f"   YouTube Email: {settings.youtube_email}")
    print(f"   YouTube Password: {'*' * len(settings.youtube_password) if settings.youtube_password else '未设置'}")
    ...
    try:
        ...
        if auth_bundle:
            ...
            print(f"   PO Token: {'✅ 已获取' if auth_bundle.po_token else '❌ 未获取'}")
            print(f"   Visitor Data: {'✅ 已获取' if auth_bundle.visitor_data else '❌ 未获取'}")
            print(f"   Cookie File: {auth_bundle.cookie_file_path}")
            
            if auth_bundle.po_token:
                print(f"   PO Token (前20字符): {auth_bundle.po_token[:20]}...")
                print(f"   PO Token (格式化): {auth_bundle.get_formatted_po_token()[:25]}...")
        ...
        if auth_bundle and auth_bundle.has_tokens():
            ...
            if 'youtube' in extractor_args:
                ...
                if 'visitor_data' in youtube_config:
                    print(f"   Visitor Data配置: {youtube_config['visitor_data'][:20]}...")
