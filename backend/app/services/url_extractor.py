...
class ExtractedVideo:
    ...
    def get_format_by_height(self, height: int) -> Optional[VideoFormat]:
        ...
        video_only = [
            f for f in self._formats if f.is_video_only and f.url and f.height
        ]
        if video_only:
    ...
    def get_download_urls(
        self, resolution: str = "720", prefer_mp4: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        def select_best_format(formats: list, target_height: int, prefer_mp4: bool):
            ...
            candidates = sorted(
                formats, key=lambda f: abs((f.height or 0) - target_height)
            )
            min_diff = abs((candidates[0].height or 0) - target_height)
            closest = [
                f
                for f in candidates
                if abs((f.height or 0) - target_height) == min_diff
            ]
        ...
        if resolution == "audio":
            audio_formats = [
                f for f in self._formats if f.is_audio_only and f.is_usable
            ]
            if audio_formats:
                # Prefer direct download, then m4a
                audio_formats.sort(
                    key=lambda f: (f.is_direct_download, f.ext == "m4a", f.tbr or 0),
                    reverse=True,
                )
        ...
        combined_formats = [
            f for f in self._formats if f.has_both and f.is_usable and f.height
        ]
        video_only_formats = [
            f for f in self._formats if f.is_video_only and f.is_usable and f.height
        ]
        audio_only_formats = [
            f for f in self._formats if f.is_audio_only and f.is_usable
        ]
        ...
        best_combined = (
            select_best_format(combined_formats, target_height, prefer_mp4)
            if combined_formats
            else None
        )
        best_video_only = (
            select_best_format(video_only_formats, target_height, prefer_mp4)
        ...
        if best_combined and not best_video_only:
            ...
            if target_height >= 720:
                if (
                    video_only_height >= target_height
                    and combined_height < target_height
                ):
                ...
                use_combined = (
                    ...
                    or combined_height >= video_only_height
                )
...
class YouTubeURLExtractor:
    ...
    def _build_opts(self, strategy: int = 1) -> Dict[str, Any]:
        ...
        if self._auth_bundle and self._auth_bundle.visitor_data:
            ...
            logger.info(
                f"Using visitor data (length: {len(self._auth_bundle.visitor_data)})"
            )
    ...
    async def extract(self, url: str) -> ExtractedVideo:
        ...
        raise ExtractionError(
            f"All extraction strategies failed: {'; '.join(errors[-3:])}"
        )
    ...
    async def _try_agentgo_fallback(self, url: str) -> Optional[ExtractedVideo]:
        ...
        try:
            ...
            logger.info(
                f"Fetching authentication via AgentGo (region: {self.region or 'default'})..."
            )
            auth_bundle = await service.get_youtube_authentication_bundle(
                force_refresh=True, region=self.region, video_url=url
            )
            ...
            logger.info(
                f"Got fresh auth: cookies={bool(auth_bundle.cookies)}, visitor_data={bool(auth_bundle.visitor_data)}"
            )
