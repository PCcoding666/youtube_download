...
class TokenExtractionMetrics:
    ...
    def record_extraction_attempt(self, region: str, start_time: float):
        ...
        self.logger.debug(f"Starting extraction attempt #{self._extraction_attempts} for region: {region}")
    ...
    def record_extraction_success(self, region: str, duration: float, has_po_token: bool, has_visitor_data: bool):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        success_rate = (self._successful_extractions / self._extraction_attempts * 100) if self._extraction_attempts > 0 else 0
        avg_extraction_time = (self._total_extraction_time / self._successful_extractions) if self._successful_extractions > 0 else 0
        po_token_rate = (self._po_token_successes / self._successful_extractions * 100) if self._successful_extractions > 0 else 0
        visitor_data_rate = (self._visitor_data_successes / self._successful_extractions * 100) if self._successful_extractions > 0 else 0
...
class TokenExtractor:
    ...
    def _log_secure(self, level: int, message: str, sensitive_data: Optional[Dict[str, Any]] = None):
        """
        ...
        if sensitive_data:
            ...
            for key, value in sensitive_data.items():
                if key.lower() in ['token', 'po_token', 'password', 'api_key', 'secret']:
                    if value:
                        # Show only length and first/last few characters for debugging
                        if isinstance(value, str) and len(value) > 8:
                            sanitized[key] = f"[{len(value)} chars: {value[:3]}...{value[-3:]}]"
                        else:
    ...
    async def extract_po_token(self, page, video_url: Optional[str] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            def handle_request(request):
                try:
                    ...
                    if '/youtubei/v1/player' in url or '/youtubei/v1/next' in url:
                        self.logger.info(f"🔍 Found player/next API request: {url[:80]}...")
                        
                        # Get request body (POST data)
                        post_data = request.post_data
                        if post_data:
                            self.logger.debug(f"Request has POST data (length: {len(post_data)})")
                            try:
                                body = json.loads(post_data)
                                # Look for serviceIntegrityDimensions.poToken
                                service_integrity = body.get('serviceIntegrityDimensions', {})
                                po_token = service_integrity.get('poToken')
                                
                                if po_token:
                                    self.logger.info(f"Found poToken in serviceIntegrityDimensions (length: {len(po_token)})")
                                    if self.validate_po_token(po_token):
                                        timestamp = time.time()
                                        self._extracted_po_tokens.append((po_token, timestamp))
                                        
                                        token_hash = self._create_token_hash(po_token)
                                        self._log_secure(
                                            logging.INFO,
                                            f"✅ Found PO token in player API request body (hash: {token_hash})",
                                            {'token_length': len(po_token), 'timestamp': timestamp}
                                        )
                                    else:
                                        self.logger.warning(f"PO token validation failed (length: {len(po_token)})")
                                else:
                                    self.logger.debug("No poToken in serviceIntegrityDimensions")
                            except json.JSONDecodeError as e:
                                self.logger.debug(f"Failed to parse POST data as JSON: {e}")
                        else:
                            self.logger.debug("No POST data in player request")
                    
                    # Method 2: Look for PO token in URL parameters (pot=)
                    if "pot=" in url:
                        self.logger.info(f"🔍 Found request with pot parameter: {url[:100]}...")
                        ...
                        if 'pot' in query_params:
                            ...
                            if pot_value and self.validate_po_token(pot_value):
                                ...
                                self._log_secure(
                                    ...
                                    {'token_length': len(pot_value), 'timestamp': timestamp}
                                )
                    
                    # Method 3: Look for PO token in googlevideo.com requests (GVS)
                    if 'googlevideo.com' in url:
                        self.logger.debug(f"🔍 Found googlevideo request: {url[:100]}...")
                        if 'pot=' in url:
                            parsed_url = urllib.parse.urlparse(url)
                            query_params = urllib.parse.parse_qs(parsed_url.query)
            ...
            if "youtube.com" in current_url and "/watch" in current_url:
                self.logger.info(f"Already on YouTube video page: {current_url[:50]}...")
                ...
                if video_url and video_url.startswith(('https://www.youtube.com/watch', 'https://youtu.be/')):
                    target_url = video_url
                    self.logger.info(f"Using provided video URL for token extraction: {target_url}")
                else:
                ...
                if not navigation_success:
                    for fallback_url in ['https://www.youtube.com', 'https://m.youtube.com']:
                        try:
            ...
            try:
                self.logger.info("Attempting to trigger video playback for PO token generation...")
            ...
            if self._extracted_po_tokens:
                ...
                self._log_secure(
                    ...
                    f"PO token extraction completed successfully (operation: {operation_id})",
                    {
                        ...
                        'selected_token_age': round(time.time() - self._extracted_po_tokens[0][1], 2)
                    }
                )
            ...
            self._metrics.record_extraction_error("unknown", "po_token_extraction", str(e))
            return None
    
    async def extract_visitor_data(self, page, video_url: Optional[str] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            if navigation_needed:
                ...
                if video_url and video_url.startswith(('https://www.youtube.com/watch', 'https://youtu.be/')):
                    target_url = video_url
                    self.logger.info(f"Using provided video URL for visitor data extraction: {target_url}")
                else:
                    target_url = "https://www.youtube.com/watch?v=_MLwQUebJUc"  # Fallback video for better visitor data extraction
                    if video_url:
                        self.logger.warning(f"Invalid video URL provided: {video_url}, using fallback for visitor data")
                    else:
                        self.logger.info("No video URL provided, using fallback video for visitor data extraction")
                
                # Strategy 1: Direct navigation to YouTube video with extended timeout
                try:
                    self.logger.info(f"Navigating to YouTube video for visitor data extraction: {target_url}")
                    await page.goto(target_url, wait_until='domcontentloaded', timeout=60000)
                    navigation_success = True
                    self.logger.info("Navigation to video successful for visitor data extraction")
                except Exception as e:
                    self.logger.warning(f"Direct navigation to video failed for visitor data: {e}")
                
                # Strategy 2: Fallback to homepage if video failed (with extended timeout)
                if not navigation_success:
                    try:
                        self.logger.info("Falling back to YouTube homepage for visitor data extraction...")
                        await page.goto('https://www.youtube.com', wait_until='domcontentloaded', timeout=60000)
                        navigation_success = True
                        self.logger.info("Homepage navigation successful for visitor data")
                    except Exception as e:
                        self.logger.warning(f"Homepage navigation failed for visitor data: {e}")
                
                # Strategy 3: Try mobile YouTube if desktop failed (with extended timeout)
                if not navigation_success:
                    try:
                        self.logger.info("Trying mobile YouTube for visitor data extraction...")
                        await page.goto('https://m.youtube.com', wait_until='domcontentloaded', timeout=60000)
                        navigation_success = True
                        self.logger.info("Mobile YouTube navigation successful")
                    except Exception as e:
                        self.logger.warning(f"Mobile navigation failed for visitor data: {e}")
                
                if not navigation_success:
                    self.logger.error("All navigation strategies failed for visitor data extraction")
                    return None
            ...
            self.logger.debug("Executing JavaScript to extract visitor data using multiple methods...")
            ...
            self._metrics.record_extraction_error("unknown", "visitor_data_extraction", str(e))
            return None
    ...
    async def _extract_visitor_data_js_only(self, page) -> Optional[str]:
        ...
        """
        try:
            self.logger.debug("Extracting visitor data using JavaScript (no navigation)...")
    ...
    async def get_browser_ip_info(self, page) -> Optional[Dict[str, str]]:
        ...
        """
        try:
            self.logger.info("Getting AgentGo browser IP information for proxy consistency...")
            ...
            if ip_info and ip_info.get('ip'):
                ...
                self.logger.info(f"AgentGo browser IP: {ip_info.get('ip')} ({ip_info.get('country', 'Unknown')})")
                return ip_info
    ...
    def validate_visitor_data(self, data: str) -> bool:
        ...
        if not data or not isinstance(data, str):
            if self._debug_enabled:
                self.logger.debug(f"Visitor data validation failed: invalid type or empty (type: {type(data)})")
            return False
        
        # Check if data is non-empty after stripping
        if len(data.strip()) == 0:
            if self._debug_enabled:
                self.logger.debug("Visitor data validation failed: empty after stripping")
            return False
        ...
        if not re.match(r'^[A-Za-z0-9_\-+=%]+$', data):
            if self._debug_enabled:
                self.logger.debug(f"Visitor data validation failed: invalid characters (length: {len(data)})")
            return False
        
        # Additional length check - visitor data is usually substantial
        if len(data) < 5:
            if self._debug_enabled:
                self.logger.debug(f"Visitor data validation failed: too short (length: {len(data)})")
            return False
        
        if self._debug_enabled:
            data_hash = self._create_token_hash(data)
            self.logger.debug(f"Visitor data validation passed (hash: {data_hash}, length: {len(data)})")
    ...
    async def extract_tokens_with_timeout(self, page, timeout: int = 90, video_url: Optional[str] = None) -> TokenExtractionResult:
        ...
        try:
            ...
            target_url = video_url if video_url and video_url.startswith(('https://www.youtube.com/watch', 'https://youtu.be/')) else "https://www.youtube.com/watch?v=_MLwQUebJUc"
            
            current_url = page.url
            if "youtube.com" not in current_url:
                self.logger.info(f"Pre-navigating to YouTube before token extraction: {target_url}")
                navigation_success = False
                ...
                for strategy_name, nav_url, wait_until in [
                    ...
                    try:
                        ...
                        self.logger.warning(f"Navigation to {strategy_name} failed: {e}")
            ...
            try:
                ...
                if visitor_data:
                    self.logger.info(f"✅ Visitor data extracted successfully (length: {len(visitor_data)})")
            ...
            self._metrics.record_extraction_error("unknown", "extraction_error", error_msg)
...
class AgentGoService:
    ...
    def _log_initialization_status(self):
        ...
        if not self.is_configured():
            ...
            self.logger.debug("AgentGo service fully configured and ready for token extraction")
    ...
    async def _connect_and_get_cookies(
        ...
        if not HAS_PLAYWRIGHT:
            raise AgentGoError("Playwright is not installed. Run: pip install playwright")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    if contexts:
                        context = contexts[0]
                        logger.info(f"Using existing context (pages: {len(context.pages)})")
                    else:
                        context = await browser.new_context()
                        logger.info("Created new browser context")
                except KeyError as ke:
                    # Handle Playwright compatibility issue with AgentGo
                    logger.warning(f"BrowserContext compatibility issue: {ke}, creating new context...")
                    context = await browser.new_context()
    ...
    async def get_youtube_authentication_bundle(
        ...
        if not HAS_PLAYWRIGHT:
            ...
            self._metrics.record_extraction_error(region, "missing_dependency", error_msg)
            return None
        ...
        if force_refresh or not cookie_file_path:
            try:
                ...
                ws_url = self._build_ws_url(region)
                self.logger.info(f"Connecting to AgentGo browser (region: {region}, operation: {operation_id})...")
                        ...
                        try:
                            ...
                            try:
                                ...
                                self._log_secure(
                                    ...
                                    {'connection_time_seconds': round(connection_duration, 2)}
                                )
                            except Exception as e:
                                self.logger.warning(f"Playwright connect failed: {e}, trying CDP...")
                            ...
                            try:
                                ...
                                if contexts:
                                    ...
                                    context = await browser.new_context()
                                    self.logger.info(f"Created new browser context (operation: {operation_id})")
                            except KeyError as ke:
                                # Handle Playwright compatibility issue with AgentGo
                                # This happens when BrowserContext initialization fails due to missing 'options'
                                self.logger.warning(f"BrowserContext compatibility issue: {ke}, creating new context...")
                                context = await browser.new_context()
                                self.logger.info(f"Created new browser context after compatibility fix (operation: {operation_id})")
                            ...
                            try:
                                ...
                                self.logger.warning(f"Error getting pages: {e}, creating new page...")
                                page = await context.new_page()
                            
                            # Extract tokens using TokenExtractor with comprehensive logging
                            self.logger.info(f"Starting token extraction (operation: {operation_id})...")
                            token_result = await self.token_extractor.extract_tokens_with_timeout(page, timeout=90, video_url=video_url)
                            
                            # Get browser IP information for proxy consistency
                            self.logger.info(f"Getting browser IP information for proxy consistency (operation: {operation_id})...")
                            ip_info = await self.token_extractor.get_browser_ip_info(page)
                            
                            # Extract cookies with logging
                            self.logger.info(f"Extracting cookies (operation: {operation_id})...")
                            ...
                            if not cookies:
                                ...
                                self._metrics.record_extraction_error(region, "no_cookies", error_msg)
                            ...
                            youtube_cookies = [
                                ...
                                if any(domain in c.get("domain", "") 
                                       for domain in ["youtube.com", "google.com", ".youtube.com", ".google.com"])
                            ]
                            
                            if not youtube_cookies:
                                self.logger.warning(f"No YouTube/Google cookies found, using all cookies (operation: {operation_id})")
                            ...
                            self._log_secure(
                                ...
                                f"Cookie extraction completed (operation: {operation_id})",
                                {
                                    ...
                                    'extraction_time_seconds': round(cookies_duration, 2)
                                }
                            )
                            
                            # Save cookies to file
                            cookie_file_path = await self.save_cookies_to_file(youtube_cookies, region=region)
                            ...
                            auth_bundle = AuthenticationBundle(
                                ...
                                browser_country=ip_info.get('country') if ip_info else None,
                            ...
                            self._log_secure(
                                ...
                                f"Authentication bundle created successfully (operation: {operation_id})",
                                {
                                    ...
                                    'token_extraction_duration': round(token_result.extraction_duration, 2)
                                }
                            )
                            ...
                            self._metrics.record_extraction_error(region, "browser_automation", str(e))
                            raise AgentGoError(error_msg)
                        finally:
                            if browser:
                                try:
                                    await browser.close()
                                    self.logger.debug(f"Browser closed successfully (operation: {operation_id})")
                                except Exception as close_error:
                                    self.logger.warning(f"Error closing browser (operation: {operation_id}): {close_error}")
                finally:
    ...
    async def get_youtube_cookies(
        ...
        Returns:
            AuthenticationBundle with cookies and tokens, or None if failed
        """
        logger.info(f"Getting YouTube authentication bundle for region: {region or self.default_region}")
        ...
        async with async_playwright() as p:
            ...
            try:
                    ...
                    try:
                        ...
                        if 'googlevideo.com' in url and response.status == 200:
                            ...
                            url_info = {
                                ...
                                'quality': params.get('quality', [''])[0] if 'quality' in params else None,
                            ...
                            if 'video' in mime and 'audio' in mime:
                                ...
                                self.logger.info(f"Captured combined stream: itag={url_info['itag']}")
                            elif 'video' in mime:
                                captured_urls['video'].append(url_info)
                                self.logger.info(f"Captured video stream: itag={url_info['itag']}")
                            elif 'audio' in mime:
                                captured_urls['audio'].append(url_info)
                                self.logger.info(f"Captured audio stream: itag={url_info['itag']}")
                ...
                try:
                    ...
                    self.logger.info(f"Extracted video info: {video_info.get('title', 'Unknown')[:50]}")
                    self.logger.info(
                    ...
                    for fmt in streams.get('adaptiveFormats', []):
                        ...
                        if fmt.get('url'):
                            ...
                            if 'video' in mime:
                                captured_urls['video'].append(url_info)
                                self.logger.info(f"Found video format: {fmt.get('qualityLabel')} (itag={fmt.get('itag')})")
                            elif 'audio' in mime:
                                captured_urls['audio'].append(url_info)
...
async def fetch_youtube_authentication_bundle(
    ...
    """
    service = get_agentgo_service()
    return await service.get_youtube_cookies(
        force_refresh=force_refresh,
        region=region
    )
...
async def get_authentication_bundle_for_region(region: str) -> Optional[AuthenticationBundle]:
    """
