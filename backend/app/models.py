...
class ProcessRequest(BaseModel):
    ...
    enable_transcription: bool = Field(
        default=True, description="Enable audio transcription"
    )
...
class AuthenticationBundle(BaseModel):
    ...
    visitor_data: Optional[str] = Field(
        default=None, description="YouTube visitor data"
    )
    ...
    cookie_file_path: Optional[str] = Field(
        default=None, description="Path to cookie file"
    )

    # IP information for proxy consistency
    browser_ip: Optional[str] = Field(
        default=None, description="IP address of AgentGo browser"
    )
    browser_country: Optional[str] = Field(
        default=None, description="Country code of AgentGo browser"
    )
    browser_location: Optional[Dict[str, str]] = Field(
        default=None, description="Detailed location info of AgentGo browser"
    )
...
class TokenExtractionResult(BaseModel):
    ...
    visitor_data: Optional[str] = Field(
        default=None, description="Extracted visitor data"
    )
    ...
    def validate_extraction_method(cls, v):
        ...
        if v not in valid_methods:
            raise ValueError(
                f"extraction_method must be one of: {', '.join(valid_methods)}"
            )
        return v
...
class ExtractURLRequest(BaseModel):
    ...
    youtube_url: str = Field(..., description="YouTube video URL")
    resolution: VideoResolution = Field(
        default=VideoResolution.RES_720P, description="Preferred resolution"
    )
...
class DownloadURLs(BaseModel):
    ...
    video_url: Optional[str] = Field(
        None, description="Direct video download URL (googlevideo.com)"
    )
    audio_url: Optional[str] = Field(
        None, description="Direct audio download URL (if separate)"
    )
    video_format: Optional[VideoFormatInfo] = None
    audio_format: Optional[VideoFormatInfo] = None
    needs_merge: bool = Field(
        False, description="Whether video and audio need to be merged client-side"
    )
...
class ExtractURLResponse(BaseModel):
    ...
    download_urls: Optional[DownloadURLs] = Field(
        None, description="Direct download URLs"
    )
    all_formats: Optional[List[VideoFormatInfo]] = Field(
        None, description="All available formats"
    )
    error_message: Optional[str] = Field(None, description="Error message if failed")
    extraction_time: Optional[float] = Field(
        None, description="Time taken for extraction in seconds"
    )
    ...
    agentgo_region: Optional[str] = Field(
        None, description="AgentGo region used for authentication"
    )
    auth_method: Optional[str] = Field(
        None, description="Authentication method used (agentgo/cookies/none)"
    )
