...
async def test_architecture(video_url: str, resolution: str = '720'):
    ...
    if service.is_api_configured():
        try:
            ...
            if auth_bundle:
                ...
                print(f'  Cookies: {len(auth_bundle.cookies) if auth_bundle.cookies else 0}')
                print(f'  Visitor Data: {len(auth_bundle.visitor_data) if auth_bundle.visitor_data else 0} chars')
        except Exception as e:
    ...
    print(f'  视频: {vf.get("height")}p {vf.get("ext")} direct={vf.get("is_direct_download")}')
    ...
    probe_result = subprocess.run(
        ['ffprobe', '-v', 'error', '-select_streams', 'v:0',
         '-show_entries', 'stream=width,height,codec_name', '-of', 'json', output_file],
    ...
    if auth_bundle and auth_bundle.browser_ip:
        if auth_bundle.browser_ip == proxy_ip:
            ...
            print(f'     ⚠️ AgentGo IP ({auth_bundle.browser_ip}) ≠ 代理 IP ({proxy_ip})')
