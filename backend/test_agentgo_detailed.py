...
async def test_agentgo_api_directly():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # 尝试连接到API端点
            test_url = (
                f"{api_url}/health" if "/api" in api_url else f"{api_url}/api/health"
            )
            ...
            # 构建连接参数
            options = {"_apikey": api_key, "_region": "us", "_disable_proxy": False}
            ...
            try:
                ...
                async with websockets.connect(
                    full_ws_url, open_timeout=15, close_timeout=5
                ) as _:
                    print("   ✅ WebSocket连接成功")
                    return True
            except Exception as e:
