...
class TestAgentGoServiceIntegration:
    ...
    async def test_get_youtube_cookies_returns_authentication_bundle(self):
        ...
        with patch.object(
            service, "get_youtube_authentication_bundle", new_callable=AsyncMock
        ) as mock_method:
    ...
    async def test_get_youtube_cookies_file_path_backward_compatibility(self):
        ...
        with patch.object(
            service, "get_youtube_cookies", new_callable=AsyncMock
        ) as mock_method:
            mock_method.return_value = mock_bundle
    ...
    async def test_convenience_functions_backward_compatibility(self):
        ...
        with patch(
            "app.services.agentgo_service.get_agentgo_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_service.get_youtube_cookies_file_path = AsyncMock(
                return_value="/tmp/test_cookies.txt"
            )
