...
class TestTokenExtractor:
    ...
    def test_validate_po_token_invalid(self):
        ...
        for token in invalid_tokens:
            assert not extractor.validate_po_token(token), (
    ...
    def test_validate_visitor_data_valid(self):
        ...
        for data in valid_data:
            assert extractor.validate_visitor_data(data), (
                f"Visitor data should be valid: {data}"
            )
    ...
    def test_validate_visitor_data_invalid(self):
        ...
        for data in invalid_data:
            assert not extractor.validate_visitor_data(data), (
                f"Visitor data should be invalid: {data}"
            )
...
class TestTokenExtractionProperties:
    ...
    async def test_property_network_monitoring_and_javascript_execution_during_navigation(
        ...
        video_urls,
        navigation_success,
        has_network_requests,
        has_javascript_context,
    ):
        """
        ...
        **Validates: Requirements 1.1, 2.1**
        ...
        if navigation_success:
            ...
            mock_page.goto = AsyncMock(side_effect=Exception("Navigation failed"))
            mock_page.evaluate = AsyncMock(
                side_effect=Exception("JavaScript execution failed")
            )
        ...
        if has_javascript_context and navigation_success:
            ...
            mock_page.evaluate.return_value = (
                "test_visitor_data_123" if has_javascript_context else None
            )
        ...
        visitor_data_result = await extractor.extract_visitor_data(
            mock_page, video_urls
        )
        ...
        assert network_monitoring_setup, (
            "Network request monitoring should be set up during navigation"
        )
        ...
        if navigation_success:
            ...
            assert mock_page.goto.call_count >= 1, (
                "Navigation should be attempted during token extraction"
            )

            # JavaScript execution should be attempted for visitor data
            if has_javascript_context:
                assert mock_page.evaluate.call_count >= 1, (
                    "JavaScript execution should be attempted for visitor data extraction"
                )
        ...
        assert True, (
            "Both extraction methods should complete without raising exceptions"
        )
