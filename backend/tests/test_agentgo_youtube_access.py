...
class AgentGoSessionClient:
    ...
    def _build_ws_url(self, region: str = "us", session_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            async with websockets.connect(
                ...
                try:
                    ...
                    logger.info(f"Received initial message: {str(initial_msg)[:200]}...")
                except asyncio.TimeoutError:
                    logger.info("No initial message (this is normal for Playwright CDP)")
    ...
    async def test_multiple_regions(self) -> Dict[str, SessionTestResult]:
        ...
        for region in regions_to_test:
            ...
            logger.info(f"{status} Region {region}: connected={result.session_connected}, "
...
class TestAgentGoSession:
    ...
    async def test_websocket_connection(self, client):
        ...
        logger.info(f"WebSocket test: success={result.success}, "
    ...
    async def test_session_persistence(self, client):
        ...
        logger.info(f"Persistence test: success={result.success}, "
                   f"duration={result.duration_ms:.0f}ms")
...
async def run_all_tests():
    ...
    print("\n" + "=" * 70)
    all_passed = (
        region_results.get("us", SessionTestResult(False, "us", 0)).success and
        successful >= 2
    )
