...
async def test_extract_and_merge():
    ...
    if service.is_configured():
        ...
        if auth_bundle:
            print(
                f"    ✅ Visitor Data: {auth_bundle.visitor_data[:30] if auth_bundle.visitor_data else 'None'}..."
            )
    ...
    try:
        ...
        if urls.get("video_format"):
            ...
            print(
                f"    视频格式: {vf.get('height')}p {vf.get('ext')} {vf.get('vcodec')}"
            )
    ...
    video_size = (
        urls.get("video_format", {}).get("filesize")
        if urls.get("video_format")
        else None
    )
    audio_size = (
        urls.get("audio_format", {}).get("filesize")
        if urls.get("audio_format")
        else None
    )
    if video_size:
            ...
            if ready:
                ...
                if line:
                    ...
                    if "size=" in line:
                        ...
                        try:
                            ...
                            if "kB" in size_part:
                                ...
                                if elapsed > 0:
                                    ...
                                    if total_size_mb > 0:
                                        progress = min(
                                            100, (current_mb / total_size_mb) * 100
                                        )
                                        eta = (
                                            (total_size_mb - current_mb)
                                            / (avg_speed / 1024)
                                            if avg_speed > 0
                                            else 0
                                        )
                                        print(
                                            f"\r    📊 {current_mb:.1f}/{total_size_mb:.1f} MB ({progress:.0f}%) | {avg_speed:.0f} KB/s | ETA: {eta:.0f}s   ",
                                            end="",
                                            flush=True,
                                        )
                                    else:
                                        ...
                                        print(
                                            f"\r    📊 {current_mb:.1f} MB | {avg_speed:.0f} KB/s | {elapsed:.0f}s   ",
                                            end="",
                                            flush=True,
                                        )
                        except Exception:
                        ...
                        "error" in line.lower() and "error=" not in line.lower()
                ...
                if no_progress > 3:
                    print(
                        f"\r    ⏳ 等待中... {elapsed:.0f}s (无进度: {no_progress:.0f}s)   ",
                        end="",
                        flush=True,
                    )
