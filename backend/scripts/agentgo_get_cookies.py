...
async def test_ytdlp_with_cookies(
    cookie_file: str, video_url: str = "https://www.youtube.com/watch?v=jNQXAC9IVRw"
):
...
async def main():
    parser = argparse.ArgumentParser(
        description="Extract YouTube cookies using AgentGo"
    )
    ...
    parser.add_argument(
        "--region", default="us", help="AgentGo region (us, uk, de, jp, etc.)"
    )
    parser.add_argument(
        "--output", default="/tmp/youtube_cookies.txt", help="Cookie file output path"
    )
    parser.add_argument(
        "--test", action="store_true", help="Test yt-dlp after extraction"
    )
    parser.add_argument(
        "--video",
        default="https://www.youtube.com/watch?v=jNQXAC9IVRw",
        help="Video URL for testing",
    )
    ...
    extractor = AgentGoCookieExtractor(
        api_key=api_key, region=args.region, cookie_file=args.output
    )
    ...
    cookie_file = await extractor.run(login=args.login, email=email, password=password)
